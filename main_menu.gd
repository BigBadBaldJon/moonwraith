...
@onready var mappings_container: VBoxContainer = $CenterContainer/OptionsPanel/VBox/MappingScroll/MappingsContainer
@onready var remap_hint_label: Label = $CenterContainer/OptionsPanel/VBox/RemapHintLabel
...
func _build_input_mapping_rows() -> void:
	for child: Node in mappings_container.get_children():
		child.queue_free()

	action_buttons.clear()

	for mapping_entry: Dictionary in REMAPPABLE_ACTIONS:
		var action_name: StringName = mapping_entry.get("action", &"") as StringName
		var display_name: String = String(mapping_entry.get("label", ""))
		...
		var remap_button: Button = Button.new()
...
func _refresh_action_button_texts() -> void:
	for mapping_entry: Dictionary in REMAPPABLE_ACTIONS:
		...
		var remap_button: Button = action_buttons.get(action_name) as Button
...
func _get_action_event_text(action_name: StringName) -> String:
	...
	if event is InputEventMouseButton:
		...
		return "Mouse " + _get_mouse_button_name(mouse_event.button_index)

	return event.as_text()


func _begin_remap(action_name: StringName) -> void:
...
func _apply_remapped_input(action_name: StringName, input_event: InputEvent) -> void:
...
func _update_remap_hint() -> void:
	...
	var action_label: String = String(remapping_action).replace("_", " ").capitalize()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey:
		...
		_apply_remapped_input(remapping_action, key_event)
		get_viewport().set_input_as_handled()
		return
	...
	if event is InputEventMouseButton:
		...
		_apply_remapped_input(remapping_action, mouse_event)
		get_viewport().set_input_as_handled()
